                            ...
                            (Grid(coldefs = [ Star; Absolute 5.0; Star; Absolute 5.0; Star ], rowdefs = [ Star; Absolute 5.0; Star; Absolute 5.0; Star ]) {
                                ...
                                for row, col as pos in positions do
                                    if canPlay model model.Board[pos] then
                                        ...
                                        | Empty -> ()
                                        | Full X ->
                                            Label("X").font(size = model.VisualBoardSize / 3.).centerText().margin(10.).gridRow(row * 2).gridColumn(col * 2)

                                        | Full O ->
                                            Label("O").font(size = model.VisualBoardSize / 3.).centerText().margin(10.).gridRow(row * 2).gridColumn(col * 2)
                            })
                                .rowSpacing(0.)
                                .columnSpacing(0.)
                                .centerVertical()
