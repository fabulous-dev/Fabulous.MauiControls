...
type IFabComponentWebView =
    inherit IFabComponentView
    inherit IFabWebView

module WebView =
    let Navigated =
        ComponentAttributes.defineEvent<WebNavigatedEventArgs> "WebView_Navigated" (fun target -> (target :?> WebView).Navigated)

    let Navigating =
        ComponentAttributes.defineEvent<WebNavigatingEventArgs> "WebView_Navigating" (fun target -> (target :?> WebView).Navigating)
...
module WebViewBuilders =
    type Fabulous.Maui.Components.View with
        ...
        static member inline WebView<'msg>(source: WebViewSource) =
            WidgetBuilder<'msg, IFabComponentWebView>(WebView.WidgetKey, WebView.Source.WithValue(source))
...
type WebViewModifiers() =
    ...
    [<Extension>]
    static member inline onNavigated(this: WidgetBuilder<'msg, #IFabComponentWebView>, fn: WebNavigatedEventArgs -> unit) =
        this.AddScalar(WebView.Navigated.WithValue(fn))

    /// <summary>Listen for the Navigating event</summary>
    /// <param name="this">Current widget</param>
    /// <param name="fn">Message to dispatch</param>
    [<Extension>]
    static member inline onNavigating(this: WidgetBuilder<'msg, #IFabComponentWebView>, fn: WebNavigatingEventArgs -> unit) =
        this.AddScalar(WebView.Navigating.WithValue(fn))

    /// <summary>Link a ViewRef to access the direct WebView control instance</summary>
    /// <param name="this">Current widget</param>
    /// <param name="value">The ViewRef instance that will receive access to the underlying control</param>
    [<Extension>]
    static member inline reference(this: WidgetBuilder<'msg, IFabComponentWebView>, value: ViewRef<WebView>) =
