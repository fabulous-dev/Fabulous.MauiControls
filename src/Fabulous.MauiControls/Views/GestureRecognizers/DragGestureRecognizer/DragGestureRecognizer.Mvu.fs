...
module DragGestureRecognizerMvu =
    let DragStarting =
        Attributes.Mvu.defineEvent<DragStartingEventArgs> "DragGestureRecognizerMvu_DragStarting" (fun target -> (target :?> DragGestureRecognizer).DragStarting)

    let DropCompleted =
        Attributes.Mvu.defineEvent<DropCompletedEventArgs> "DragGestureRecognizerMvu_DropCompleted" (fun target -> (target :?> DragGestureRecognizer).DropCompleted)
