...
module TitleBar =
    ...
    let ForegroundColor =
        Attributes.defineBindableColor TitleBar.ForegroundColorProperty
    ...
    let Subtitle =
        Attributes.defineBindableWithEquality<string> TitleBar.SubtitleProperty

    let Title = Attributes.defineBindableWithEquality<string> TitleBar.TitleProperty

    let TrailingContent =
        Attributes.defineBindableWidget TitleBar.TrailingContentProperty

[<AutoOpen>]
module TitleBarBuilders =
    type Fabulous.Maui.View with
        static member inline TitleBar(icon: ImageSource, title: string) =
            WidgetBuilder<'msg, IFabTitleBar>(TitleBar.WidgetKey, TitleBar.Icon.WithValue(ImageSourceValue.Source icon), TitleBar.Title.WithValue(title))

        static member inline TitleBar(icon: string, title: string) =
            WidgetBuilder<'msg, IFabTitleBar>(TitleBar.WidgetKey, TitleBar.Icon.WithValue(ImageSourceValue.File icon), TitleBar.Title.WithValue(title))

        static member inline TitleBar(icon: Uri, title: string) =
            WidgetBuilder<'msg, IFabTitleBar>(TitleBar.WidgetKey, TitleBar.Icon.WithValue(ImageSourceValue.Uri icon), TitleBar.Title.WithValue(title))

        static member inline TitleBar(icon: Stream, title: string) =
            WidgetBuilder<'msg, IFabTitleBar>(TitleBar.WidgetKey, TitleBar.Icon.WithValue(ImageSourceValue.Stream icon), TitleBar.Title.WithValue(title))
